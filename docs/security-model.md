...
| Compromised daemon phones home | nftables egress rules drop non-API outbound |

---

## Network Egress Control

The nullbot user is restricted to outbound connections to the configured LLM API endpoint only. All other egress is dropped by nftables firewall rules scoped to the `nullbot` UID.
...
Blocked traffic:
- All other outbound connections from the `nullbot` user
- Arbitrary HTTP/HTTPS to non-API hosts
- Raw TCP/UDP to any port other than DNS and approved HTTPS
